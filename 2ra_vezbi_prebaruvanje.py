...
class Pillars(Problem):
    ...
    def successor(self, state):
        ...
        for source_idx in range(self.num_pillars):
            if pillars[source_idx]:
                disk = pillars[source_idx][-1]

                for dest_idx in range(self.num_pillars):
                    if source_idx != dest_idx:
                        if not pillars[dest_idx] or disk <= pillars[dest_idx][-1]:
                            new_pillars = [list(p) for p in pillars]

                            new_pillars[source_idx].pop()

                            new_pillars[dest_idx].append(disk)

                            new_state = tuple(tuple(p) for p in new_pillars)

                            action = f"MOVE TOP BLOCK FROM PILLAR {source_idx + 1} TO PILLAR {dest_idx + 1}"
...
if __name__ == "__main__":
    ...
    for i in range(len(initial_input)):
        if initial_input[i]:
            ...
            pillar = tuple()
        initial_pillars.append(pillar)

    initial_state = tuple(initial_pillars)
    ...
    for i in range(len(initial_pillars)):
        if goal_input[i]:
            ...
            pillar = tuple()
        goal_pillars.append(pillar)

    goal_state = tuple(goal_pillars)

    pillars = Pillars(initial_state, goal_state)

    solution = breadth_first_graph_search(pillars)
